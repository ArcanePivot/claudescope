(() => {
    ...
    const decodeUsageRowsV3 = (rows, source, opts = {}) => {
    ...
    const ensureRawData = () => {
        if (data.records || data.recordsV2 || data.recordsV3 || window.CLAUDESCOPE_RAW_DATA)
    ...
    const pricingSourceDisplay = () => {
        ...
        if (pricingSource === "builtin")
            return "内置 Anthropic 公开价";
        if (pricingSource.startsWith("user-config:"))
            return "用户配置 ~/.claude-scope/pricing.json";
        if (pricingSource.startsWith("user-config-broken"))
            return "用户配置损坏，已回退内置";
        if (pricingSource.startsWith("user-config-empty"))
            return "用户配置为空，已回退内置";
    ...
    document.querySelectorAll(".subagent-mode").forEach((button) => {
        button.addEventListener("click", () => {
            const wantExpanded = button.dataset.subagentMode === "expanded";
            if (uiState.sessionsExpandSubagents === wantExpanded)
                return;
            uiState.sessionsExpandSubagents = wantExpanded;
            document.querySelectorAll(".subagent-mode").forEach((b) => {
