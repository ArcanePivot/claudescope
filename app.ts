interface Window {
  CLAUDESCOPE_DATA?: any;
  CLAUDESCOPE_RAW_DATA?: any;
  CODEXSCOPE_DATA?: any;
  CODEXSCOPE_RAW_DATA?: any;
  QUOTASCOPE_DATA?: any;
  CLAUDESCOPE_SAMPLE_DATA?: any;
}

(() => {
  const data = window.CLAUDESCOPE_DATA || window.CODEXSCOPE_DATA || window.QUOTASCOPE_DATA || window.CLAUDESCOPE_SAMPLE_DATA;
  if (!data) return;
  const uiState = {
    sessionsExpanded: false,
    modelsExpanded: false,
    sessionsExpandSubagents: false, // M4：[聚合主会话] / [展开子代理] toggle
    sessionMode: "tokens",
  ...
  const getRawData = () => window.CLAUDESCOPE_RAW_DATA || window.CODEXSCOPE_RAW_DATA || data;
  ...
  // V3 数据自带 sourceSidIdx + flags（10 列），V2 没有。
  // 用来决定是否显示 [聚合主会话] / [展开子代理] toggle —— V2 数据下 toggle 无效，直接隐藏。
  const hasSubagentColumn = () => {
    const v3Source = sourceWithRows("recordsV3");
    if (Array.isArray(v3Source.recordsV3) && v3Source.recordsV3.length) {
      return Array.isArray(v3Source.recordsV3[0]) && v3Source.recordsV3[0].length >= 10;
    }
    return false;
  };
  // V3 flag bits（与 generator/parser 钦定一致）
  const V3_FLAG_IS_SUBAGENT = 0x01;
  const V3_FLAG_WAS_SYNTHETIC = 0x02;
  const V3_FLAG_IS_FAILURE = 0x04;
  const V3_FLAG_HAS_ERROR = 0x08;
  const V3_FLAG_IS_THIRD_PARTY = 0x10;
  const decodeStats = {
    thirdPartyExcluded: 0,
    failureExcluded: 0,
  };
  const decodeUsageRowsV2 = (rows, source) => {
    const dataset = rawDataset(source);
    return rows.map((row) => {
      const sid = dataset.sidAt(row?.[1]);
      // V2 没有 sourceSid / flags 列 — 用 sid 自身填 idx=8、flags=0 填 idx=9，
      // 让下游 record[8] 不会越界；但 V2 路径下 UI 会隐藏 [展开子代理] toggle，
      // 即便强行触发也只是退化为按 sid 分组（与默认聚合一致），不会丢数据。
      return [
        dataset.ts(row?.[0]),
        sid,
  ...
  // V3 → 内部 V2 形式映射：
  //   V3 列：[tsDelta, sidIdx, modelIdx, input, cacheRead, cacheCreate, output, total, sourceSidIdx, flags]
  //   内部 V2 列：[ts, sid, model, input, cacheRead, output, cacheCreate, total, sourceSid, flags]
  //   即把 V3 的 cacheCreate（V3[5]）放到内部 V2 的 reasoning 槽（idx=6）。
  //   reasoning 槽在 ClaudeScope 里被复用为 cacheCreate（Anthropic 不再有独立 reasoning_output_tokens）。
  //   M4 起新增 idx=8 sourceSid（subagent 自己的 catalog 名），idx=9 flags（位标记）。
  //   sid（idx=1）= 合并后主 session 名；展开子代理 toggle 时换用 sourceSid。
  const decodeUsageRowsV3 = (rows, source, opts: any = {}) => {
    const excludeThirdParty = opts.excludeThirdParty !== false;
    const dataset = rawDataset(source);
    const out: any[] = [];
    decodeStats.thirdPartyExcluded = 0;
    decodeStats.failureExcluded = 0;
    for (let i = 0; i < rows.length; i += 1) {
      const row = rows[i] || [];
      const flags = Number(row[9]) || 0;
      if (flags & V3_FLAG_IS_FAILURE) {
        decodeStats.failureExcluded += 1;
        continue;
      }
      if (excludeThirdParty && (flags & V3_FLAG_IS_THIRD_PARTY)) {
        decodeStats.thirdPartyExcluded += 1;
        continue;
      }
      const input = Number(row[3]) || 0;
      const cacheRead = Number(row[4]) || 0;
      const cacheCreate = Number(row[5]) || 0;
      const output = Number(row[6]) || 0;
  ...
  const decodeFailureRowsV3 = (rows, source) => {
    const dataset = rawDataset(source);
    return rows.map((row) => [
      dataset.ts(row?.[0]),
      dataset.sidAt(row?.[1]),
  ...
  const decodeRecords = () => {
    // V3 优先（ClaudeScope 主格式），否则降级到 V2，最后兜底到旧 records
    const v3Source = sourceWithRows("recordsV3");
    if (Array.isArray(v3Source.recordsV3)) {
      return sortByTime(decodeUsageRowsV3(v3Source.recordsV3, v3Source));
    }
  ...
  const decodeFailureRecords = () => {
    const v3Source = sourceWithRows("failureRecordsV3");
    if (Array.isArray(v3Source.failureRecordsV3)) {
      return sortByTime(decodeFailureRowsV3(v3Source.failureRecordsV3, v3Source));
    }
  ...
  const ensureRawData = () => {
    if (data.records || data.recordsV2 || data.recordsV3 || window.CLAUDESCOPE_RAW_DATA) return Promise.resolve();
    if (!rawDataPromise) {
      rawDataPromise = new Promise<void>((resolve, reject) => {
        ...
        script.async = true;
        script.onload = () => {
          if (!window.CLAUDESCOPE_RAW_DATA) {
            rawDataPromise = null;
  ...
  const pressureRiskRow = (window, fallbackName) => {
    if (!window || typeof window !== "object") {
      return {
        ...
        value: null,
        label: "等待数据",
        note: "本地日志暂无 token 记录",
        tone: "amber",
        percentLabel: "--",
      };
    }
    const windowLabel = window.label || fallbackName;
  ...
  const formatBucketLabel = (ts, stepMs, range) => {
    ...
    if (stepMs >= DAY) return `${date.getMonth() + 1}/${date.getDate()}`;
    if (range.end - range.start > DAY) return `${date.getMonth() + 1}/${date.getDate()} ${time}`;
    return time;
  };
  const formatStepLabel = (stepMs) => {
    if (stepMs < HOUR) return `${Math.round(stepMs / MINUTE)}分钟/点`;
    if (stepMs < DAY) return `${Math.round(stepMs / HOUR)}小时/点`;
  ...
  // ClaudeScope V3：pricing 规则有 4 个独立档位（input / cacheRead / cacheCreate / output）。
  // 兼容老 V2 数据：若只给 cached / input / output，把 cached 同时映射为 cacheRead，
  // cacheCreate 默认 0（V2 没这个字段）。
  const normalizePricingRules = (rules) => Array.isArray(rules)
    ? rules.map((rule) => {
      const patterns = (Array.isArray(rule?.patterns) ? rule.patterns : [rule?.pattern])
        ...
        .filter(Boolean);
      const input = Number(rule?.input) || 0;
      const output = Number(rule?.output) || 0;
      const cacheReadRaw = rule?.cacheRead ?? rule?.cached;
      const cacheRead = Number(cacheReadRaw) || 0;
      const cacheCreate = Number(rule?.cacheCreate) || 0;
      return {
        label: String(rule?.label || patterns[0] || "unknown"),
        patterns,
        input,
        cacheRead,
        cacheCreate,
        output,
        cached: cacheRead, // 老字段别名，保留给 V2 cost 渲染向后兼容
      };
    }).filter((rule) => rule.patterns.length && (rule.input || rule.cacheRead || rule.cacheCreate || rule.output))
    : [];
  const pricingRules = normalizePricingRules(data.pricingRules || window.CLAUDESCOPE_SAMPLE_DATA?.pricingRules);
  const pricingFallback = (() => {
    const raw = data.pricingFallback;
    if (!raw) return null;
    const out = normalizePricingRules([{ ...raw, patterns: ["__fallback__"] }])[0];
    return out || null;
  })();
  const pricingSource = data.pricingSource || (pricingRules.length ? "builtin" : "");
  const pricingSourceDisplay = () => {
    if (!pricingSource) return "未配置";
    if (pricingSource === "builtin") return "内置 Anthropic 公开价";
    if (pricingSource.startsWith("user-config:")) return "用户配置 ~/.claude-scope/pricing.json";
    if (pricingSource.startsWith("user-config-broken")) return "用户配置损坏，已回退内置";
    if (pricingSource.startsWith("user-config-empty")) return "用户配置为空，已回退内置";
  ...
  const renderPricingHelp = () => {
    ...
    if (!pricingRules.length) {
      body.innerHTML = `<tr><td colspan="5">等待价格表</td></tr>`;
      return;
    }
    body.innerHTML = pricingRules.map((rule) => `
      <tr>
        ...
        <td>${esc(priceRate(rule.cacheRead))}</td>
        <td>${esc(priceRate(rule.cacheCreate))}</td>
  ...
  const priceRecord = (record) => {
    ...
    // 内部 V2 形式 record 布局（由 decodeUsageRowsV3 映射后）：
    // [ts, sid, model, input, cacheRead, output, cacheCreate, total]
    //                                ^idx=4         ^idx=6（reasoning 槽复用为 cacheCreate）
    const model = record[2] || "unknown";
    const pricing = pricingForModel(model) || pricingFallback;
    const inputTokens = Math.max(0, Number(record[3]) || 0);
    const cacheReadTokens = Math.max(0, Number(record[4]) || 0);
    const outputTokens = Math.max(0, Number(record[5]) || 0);
    const cacheCreateTokens = Math.max(0, Number(record[6]) || 0);
    const totalTokens = inputTokens + cacheReadTokens + outputTokens + cacheCreateTokens;
    if (!pricing) {
      const result = { ...emptyCost(), unpricedTokens: totalTokens };
      recordCostCache.set(record, result);
      return result;
    }
    const multiplier = 1 / 1_000_000;
    const input = inputTokens * pricing.input * multiplier;
    const cached = cacheReadTokens * pricing.cacheRead * multiplier;
    const output = outputTokens * pricing.output * multiplier;
    const reasoning = cacheCreateTokens * pricing.cacheCreate * multiplier;
    const isFallback = pricing === pricingFallback;
    const result = {
      input,
      cached,
      output,
      ...
      pricedTokens: isFallback ? 0 : totalTokens,
      unpricedTokens: isFallback ? totalTokens : 0,
    };
    recordCostCache.set(record, result);
    return result;
  };
  ...
  const computeStats = (range) => {
    ...
    const byModel = new Map<string, { name: string; tokens: number; requests: number; cost: number; unpricedTokens: number; latencyTotal: number; latencyCount: number }>();
    const sessionCatalog = getSessionsCatalog();
    const recordCosts = new Array(filtered.length);
    // M4：默认按 sidIdx（合并后主会话）聚合；展开子代理时改用 sourceSidIdx。
    // V2 数据无 sourceSidIdx 列（toggle 已被 UI 隐藏），逻辑层再兜一层 — 永远走 record[1]。
    const expandSubagents = uiState.sessionsExpandSubagents && hasSubagentColumn();
    for (let index = 0; index < filtered.length; index += 1) {
      const record = filtered[index];
      ...
      totals.requests += 1;
      const sid = (expandSubagents ? record[8] : record[1]) || "unknown";
      const model = record[2] || "unknown";
      ...
      const modelRow = byModel.get(model) || { name: model, tokens: 0, requests: 0, cost: 0, unpricedTokens: 0, latencyTotal: 0, latencyCount: 0 };
      modelRow.tokens += record[7] || 0;
      modelRow.requests += 1;
      modelRow.cost += recordCost.total || 0;
      modelRow.unpricedTokens += recordCost.unpricedTokens || 0;
    ...
    // Phase 3C-Gate2 P1-2：未定价模型清单（按未定价 token 量降序）
    const unpricedModels = Array.from(byModel.values())
      .filter((row) => row.unpricedTokens > 0)
      .sort((a, b) => b.unpricedTokens - a.unpricedTokens)
      .map((row) => ({ name: row.name, tokens: row.unpricedTokens }));
    const duration = Math.max(1, range.end - range.start);
    ...
    const cachedReadTokens = totals.cached || 0;
    ...
    const costParts = [
      ...
      { key: "reasoning", name: "缓存写入", value: costs.reasoning, className: "cost-reasoning" },
    ].map((part) => ({
      ...part,
    ...
    return {
      ...
      summary: {
        ...
        cacheHitLabel: fmt(cachedReadTokens),
      ...
      models: models.map((row) => ({
        ...
        unpriced: (row.unpricedTokens || 0) > 0,
  ...
  const quotaSourceLabel = (withPrefix = true) => {
    ...
    if (isSampleData) {
      ...
      label = plan && plan !== "UNKNOWN" ? `Claude Code ${plan} 全局额度` : "Claude Code 全局额度";
    } else if (limitName) {
      label = `${limitName} 限额`;
    } else if (limitId) {
      label = `${limitId} 限额`;
  ...
  setText("sourcePrimary", isSampleData ? "示例数据" : "Claude Code 桌面端");
  ...
  const riskSourceDisplay = () => {
    if (!riskApplied) return "Phase 3B 未启用";
    if (!riskSourceRaw || riskSourceRaw === "builtin") return "本地压力估算 · 内置阈值";
    if (riskSourceRaw.startsWith("user-config-broken")) return "本地压力估算 · 用户配置异常（已回退内置）";
    if (riskSourceRaw.startsWith("user-config:")) return "本地压力估算 · 用户配置";
    return `本地压力估算 · ${riskSourceRaw}`;
  };
  const tertiaryLabel = isSampleData
    ? "运行脚本看真实数据"
    : (riskApplied ? riskSourceDisplay() : "Phase 3B 未启用");
  const quotaSourceText = isSampleData
    ? "示例数据"
    : (riskApplied ? `来源：${riskSourceDisplay()}` : "Phase 3B 未启用本地压力估算");
  setText("sourceTertiary", tertiaryLabel);
  setText("quotaSource", quotaSourceText);
  setText("syncText", isSampleData ? "Demo 预览" : `${data.generatedAt?.slice(11, 16) || "--"} 已同步`);

  // Phase 3B：本次扫描提示卡（dedupStats / filteredCounts / notes）
  const renderDatasetNote = () => {
    const panel = $("datasetNotePanel");
    const body = $("datasetNoteBody");
    if (!panel || !body) return;
    const dedup = data.dedupStats || {};
    const filt = data.filteredCounts || {};
    const notes = data.notes || {};
    const hasV3 = Array.isArray(data.recordsV3);
    const hasAny = hasV3 || dedup.rawUsageRows || filt.synthetic || filt.subagentRows || filt.thirdParty;
    if (!hasAny) {
      panel.hidden = true;
      return;
    }
    panel.hidden = false;
    setText("datasetNoteSource", `schema v${data.schemaVersion || "?"} · ${data.tool || "claudescope"}`);
    const cells: string[] = [];
    if (dedup.rawUsageRows !== undefined) {
      cells.push(`<div class="note-cell"><strong>${esc(fmt(dedup.keptUsageRows || 0))}</strong><span>保留 token 行（原始 ${fmt(dedup.rawUsageRows || 0)}，跨文件去重 ${fmt(dedup.duplicatesSkipped || 0)}）</span></div>`);
    }
    if (filt.synthetic) {
      cells.push(`<div class="note-cell"><strong>${esc(fmt(filt.synthetic))}</strong><span>已过滤 &lt;synthetic&gt; 占位行（不计入统计）</span></div>`);
    }
    if (filt.thirdParty) {
      cells.push(`<div class="note-cell"><strong>${esc(fmt(filt.thirdParty))}</strong><span>第三方模型 token 行（默认从主统计剥离，仅参考）</span></div>`);
    }
    if (filt.subagentRows) {
      cells.push(`<div class="note-cell"><strong>${esc(fmt(filt.subagentRows))}</strong><span>子代理行（默认合并到父主会话；可在「会话排行」切换 [展开子代理] 查看明细）</span></div>`);
    }
    if (decodeStats.thirdPartyExcluded) {
      cells.push(`<div class="note-cell"><strong>${esc(fmt(decodeStats.thirdPartyExcluded))}</strong><span>本次解码已剥离的第三方模型行（V3 flag）</span></div>`);
    }
    if (notes.disclaimer) {
      cells.push(`<div class="dataset-note-disclaimer">${esc(notes.disclaimer)}</div>`);
    }
  ...
  if (riskApplied) {
    const peakPercent = Number(pressureSummary?.primary?.peak?.percent) || 0;
    const currentPercent = Number(pressureSummary?.primary?.current?.percent) || 0;
    const stateLabel = currentPercent >= 100
      ? "压力已达阈值"
      : currentPercent >= 80
        ? "压力高位"
        : currentPercent >= 50
          ? "压力中位"
          : "压力宽松";
    setText("shieldState", `${stateLabel}（${formatWindowMinutes(pressureSummary?.primary?.windowMinutes || 300)} 峰值 ${pct(peakPercent)}）`);
  } else {
    setText("shieldState", "Phase 3B 未启用");
  }
  ...
  const seriesConfig = {
    ...
    reasoning: { label: "缓存写入", color: "#8b5cf6", width: 2.2 },
  };
  ...
  const renderModels = () => {
    ...
    $("modelList").innerHTML = head + rows.map((row, index) => {
      // P1-2：未定价模型不显示 $0.00（误导），改显「未定价」徽章 + tooltip
      const costCell = row.unpriced
        ? `<span class="model-cost unpriced" title="该模型未匹配 ~/.claude-scope/pricing.json 任何规则；未计入费用。${(row.unpricedTokens || 0).toLocaleString()} token 未定价">未定价</span>`
        : `<span class="model-cost">${esc(money(row.cost || 0))}</span>`;
      return `
      <div class="model-row${row.unpriced ? " is-unpriced" : ""}">
    ...
    $("riskList").innerHTML = rows.map((row, index) => `
      ...
      <div class="warning">
        ...
        <div>
          ...
          <span>${esc(pressureDisclaimer)}</span>
        </div>
        <span>›</span>
      </div>`;
  };
  ...
  const renderCost = () => {
    ...
    // P1-2：未定价 token / 模型醒目提示带
    const unpricedTokens = Number(cost.unpricedTokens) || 0;
    const unpricedModels = Array.isArray(cost.unpricedModels) ? cost.unpricedModels : [];
    const unpricedBanner = unpricedTokens > 0
      ? `<div class="cost-unpriced-banner" role="note">
          <strong>${esc(fmt(unpricedTokens))}</strong> token 来自 ${unpricedModels.length} 个未定价模型，已从费用估算中剥离 ——
          <span class="unpriced-models">${unpricedModels.slice(0, 3).map((m) => esc(m.name)).join("、")}${unpricedModels.length > 3 ? ` 等 ${unpricedModels.length} 个` : ""}</span>
          <small>在 <code>~/.claude-scope/pricing.json</code> 添加规则可覆盖</small>
        </div>`
      : "";
    content.innerHTML = `
      ${unpricedBanner}
      ...
      <div class="cost-footnote">
        ...
        <span>价格来源：${esc(pricingSourceDisplay())}；按本地 token × 配置价估算，实际账单和套餐额度以 Anthropic 官方为准。${esc(fxNote)}</span>
      </div>`;
  };
  ...
  // M4：[聚合主会话] / [展开子代理] toggle — 仅 V3 数据下显示 + 切换后重跑 computeStats。
  // V2 数据无 sourceSidIdx 列，toggle 默认隐藏（既保护误用，也避免出现「无效切换」假按钮）。
  const subagentToggle = $("subagentToggle");
  if (subagentToggle) {
    subagentToggle.hidden = !hasSubagentColumn();
  }
  document.querySelectorAll<HTMLElement>(".subagent-mode").forEach((button) => {
    button.addEventListener("click", () => {
      const wantExpanded = button.dataset.subagentMode === "expanded";
      if (uiState.sessionsExpandSubagents === wantExpanded) return;
      uiState.sessionsExpandSubagents = wantExpanded;
      document.querySelectorAll<HTMLElement>(".subagent-mode").forEach((b) => {
        b.classList.toggle("active", b.dataset.subagentMode === button.dataset.subagentMode);
      });
      applyRange(currentRangePreset);
    });
  });
  document.querySelectorAll<HTMLElement>(".currency-mode").forEach((button) => {
    button.addEventListener("click", () => {
  ...
  renderPricingHelp();
  setText("costSourceLabel", pricingSourceDisplay());
  applyRange("today");
